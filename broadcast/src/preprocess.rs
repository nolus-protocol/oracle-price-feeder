    ...
    Impl: impl_variant::Impl,
{
    loop {
        let cache::GetNextResult {
            ...
            ..
        }: cache::GetNextResult<Impl> = cache::get_next(requests_cache, *next_sender_id)?;
...
async fn preprocess<Impl: impl_variant::Impl>(
    ...
    let signed_tx_bytes: Vec<u8> =
        sign_and_serialize_tx(signer, node_config, hard_gas_limit, tx_body.clone())?;

    let simulation_result =
        simulate::with_signed_body(node_client, signed_tx_bytes, hard_gas_limit).await;

    let estimated_gas_limit: NonZeroU64 =
        process_simulation_result(simulation_result, fallback_gas_limit);

    let gas_limit: NonZeroU64 = adjust_gas_limit(node_config, estimated_gas_limit, hard_gas_limit);
